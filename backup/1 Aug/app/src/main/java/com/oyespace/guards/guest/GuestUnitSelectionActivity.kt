...
class GuestUnitSelectionActivity : BaseKotlinActivity() , View.OnClickListener  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rv_unit.setLayoutManager( GridLayoutManager(this@GuestUnitSelectionActivity, 2))
    }
    ...
    override fun onClick(v: View?) {
        ...
        when (v?.id) {
            ...
            R.id.buttonNext -> {
                    ...
                    val _intent = Intent(this@GuestUnitSelectionActivity, GuestBlockSelectionActivity::class.java)
    ...
    private fun managePageNumber(){
        if(arrayFullList != null && arrayFullList.size > 0){
            ...
            pageNumberAdapter =
                PaginationAdapter(this@GuestUnitSelectionActivity, pageArrayList,clickListener = {
                   page,index -> onPageClick(page,index)
                })
            rv_page.adapter = pageNumberAdapter

            rv_page.setLayoutManager(LinearLayoutManager(this@GuestUnitSelectionActivity,LinearLayoutManager.HORIZONTAL,false));
    ...
    private fun setPageData(){
        ...
        if(arrayFullList.size > LIMIT){
            ...
            rv_unit.showProgress()
            orderListAdapter =
                GuestUnitSelectionActivity.UnitListAdapter(arrayFullList, this@GuestUnitSelectionActivity, checkListener = {
                    arr,ischecked -> onCheckUnit(arr, ischecked)
                })
            rv_unit.adapter = orderListAdapter
            orderListAdapter!!.notifyDataSetChanged()
        ...
        RetrofitClinet.instance
            ...
            .subscribeWith(object : CommonDisposable<UnitsList<ArrayList<UnitPojo>>>() {
                ...
                override fun onErrorResponse(e: Throwable) {
                    dismissProgressrefresh()
                    Toast.makeText(this@GuestUnitSelectionActivity, "No Units Found ", Toast.LENGTH_LONG).show()

                }

                override fun noNetowork() {
                    dismissProgressrefresh()
                    Toast.makeText(this@GuestUnitSelectionActivity, "No network call ", Toast.LENGTH_LONG).show()
                }
            })
