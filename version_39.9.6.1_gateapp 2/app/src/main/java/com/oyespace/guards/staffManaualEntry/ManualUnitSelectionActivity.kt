...
class ManualUnitSelectionActivity : BaseKotlinActivity() , View.OnClickListener  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rv_unit.setLayoutManager( GridLayoutManager(this@ManualUnitSelectionActivity, 2))
    }
    ...
    override fun onClick(v: View?) {
        ...
        when (v?.id) {
            ...
            R.id.buttonNext -> {
                    ...
                    val _intent = Intent(this@ManualUnitSelectionActivity, ManulBlockSelectionActivity::class.java)
    ...
    private fun managePageNumber(){
        if(arrayFullList != null && arrayFullList.size > 0){
            ...
            pageNumberAdapter =
                PaginationAdapter(this@ManualUnitSelectionActivity, pageArrayList,clickListener = {
                   page,index -> onPageClick(page,index)
                })
            rv_page.adapter = pageNumberAdapter

            rv_page.setLayoutManager(LinearLayoutManager(this@ManualUnitSelectionActivity,LinearLayoutManager.HORIZONTAL,false));
    ...
    private fun setPageData(){
        ...
        if(arrayFullList.size > LIMIT){
            ...
            rv_unit.showProgress()
                orderListAdapter =
                    ManualUnitSelectionActivity.UnitListAdapter(arrayList as ArrayList<UnitPojo>, this@ManualUnitSelectionActivity, checkListener = {
            ...
            rv_unit.showProgress()
            orderListAdapter =
                ManualUnitSelectionActivity.UnitListAdapter(arrayFullList, this@ManualUnitSelectionActivity, checkListener = {
                    arr,ischecked -> onCheckUnit(arr, ischecked)
                })
            rv_unit.adapter = orderListAdapter
            orderListAdapter!!.notifyDataSetChanged()
        ...
        RetrofitClinet.instance
            ...
            .subscribeWith(object : CommonDisposable<UnitsList<ArrayList<UnitPojo>>>() {
                ...
                override fun onErrorResponse(e: Throwable) {
                    dismissProgressrefresh()
                    Toast.makeText(this@ManualUnitSelectionActivity, "No Units Found ", Toast.LENGTH_LONG).show()

                }

                override fun noNetowork() {
                    dismissProgressrefresh()
                    Toast.makeText(this@ManualUnitSelectionActivity, "No network call ", Toast.LENGTH_LONG).show()
                }
            })
