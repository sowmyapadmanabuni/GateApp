...
public class SuperRecyclerView extends FrameLayout {
    ...
    protected void initAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            mMoreProgressId = R.layout.layout_more_progress;
            mProgressId = a.getResourceId(R.styleable.superrecyclerview_layout_progress, R.layout.layout_progress);
            mEmptyText = a.getString(R.styleable.superrecyclerview_emptyText);
        } finally {
            a.recycle();
        }
    }
    ...
    private void processOnMore() {
        ...
        if (lastVisibleItemPosition > 0 && ((totalItemCount - lastVisibleItemPosition) <= ITEM_LEFT_TO_LOAD_MORE ||
    ...
    public void setEmptyViewPosition(int gravity) {
        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mEmpty.getLayoutParams();
    ...
    private void setAdapterInternal(RecyclerView.Adapter adapter, boolean compatibleWithPrevious,
        ...
        } else if (mEmptyView != null && mEmptyView instanceof ImageView) {
            ((ImageView) mEmptyView).setImageResource(emptyImage);
        }
    }
    ...
    public void setEmptyImage(int emptyId) {
        emptyImage = emptyId;
        if (mEmptyView != null && mEmptyView instanceof ImageView) {
            ((ImageView) mEmptyView).setImageResource(emptyImage);
        }
    }
    ...
    public void setEmptyAdapter(String emptyTxt, boolean isShowProgress, int emptyId) {
        mEmptyListenrId = emptyId;
        ...
        if (isShowProgress) {
            ...
            hideProgress();
            mEmpty.setVisibility(View.VISIBLE);
            setEmptyText(emptyTxt);
        }
    }


    public void setEmptyAdapter(int emptyIcon, boolean isShowProgress, int emptyId) {
        mEmptyListenrId = emptyId;
        RecyclerView.Adapter adapter = mRecycler.getAdapter();
        if (adapter != null && adapter.getItemCount() > 0) {
            return;
    ...
    public void setEmptyListener(final OnEmptyClickListener listener) {
        if (mEmptyView != null && listener != null) {
            mEmptyView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View view) {
                    listener.onEmptyItemClick(mEmptyListenrId);
                }
            });
        }
    }
